...
void directConnect(const Vector3d start_pt, const Vector3d target_pt);
...
{     
    ...
    cout<<"target is : \n "<<target_pt.transpose()<<endl;

    /*
    *
    *
    STEP 0:     function "directConnect" is used for demo
                you need to delete this function and uncomment function "pathFinding"

    *      
    */

    //directConnect(_start_pt, target_pt );
...
void directConnect(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    int np = (target_pt - start_pt).norm() * _inv_resolution;//Eigen::vector3d.norm() 模长
    Vector3d unit_vec = (target_pt - start_pt) / (double)np;
    for(int i = 0; i < np; i++)
    {
        grid_path.push_back(start_pt + i * unit_vec);
    }
    visited_nodes = grid_path;
